import type { AuthResponse, CartItem, Order, Product, SearchResult, WishlistItem } from './types'
...
export const authApi = {
  login: (email: string, password: string) => post<AuthResponse>(`${BASE}/api/auth/login`, { email, password }),
...
export const ordersApi = { create: (order: Order) => post<Order>(`${BASE}/api/orders`, order) }

export const searchApi = {
  search: (keyword: string) => get<SearchResult[]>(`${BASE}/api/search?keyword=${encodeURIComponent(keyword)}`),
}
