...
document.getElementById('btn-login')?.addEventListener('click', async () => {
    ...
    if (!EMAIL_REGEX.test(email)) {
        errorEl.textContent = 'El correo debe tener formato usuario@dominio.com';
        errorEl.classList.add('show');
        return;
    }
    try {
...
document.getElementById('btn-register')?.addEventListener('click', async () => {
    ...
    if (!username || !email || !password) {
        errorElReg.textContent = 'Completa todos los campos';
        errorElReg.classList.add('show');
        return;
    }
    if (!EMAIL_REGEX.test(email)) {
        errorElReg.textContent = 'El correo debe tener formato usuario@dominio.com';
        errorElReg.classList.add('show');
        return;
    }
    try {
